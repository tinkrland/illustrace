...
- **qlora runner** — [lora_train.py](lora_train.py): trains the spec-draft
  ...
  `--dry-run` validates a dataset anywhere; the full path is mi300x-only
  (rocm pytorch + peft, 4-bit nf4, r=64).
...
llm/
  ...
  lora_train.py      qlora runner (spec model first, judge model later)
...
- **the fine-tune corpus is small.** every build_spec.py run adds a pair, but
  "under 50 examples is smoke, thin for a real lora" per lora_train.py.
